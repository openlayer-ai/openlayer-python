...
def _extract_response_output(response_output: Any) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if isinstance(response_output, list) and response_output:
            ...
            if (hasattr(first_item, "type") and first_item.type == "function_call" and
                hasattr(first_item, "name")):
            ...
            elif (hasattr(first_item, "type") and first_item.type == "message" and
                  hasattr(first_item, "content") and first_item.content):
    ...
    try:
        ...
        if span_type == "function":
            ...
            if input_data:
                ...
                if prompt_messages:
                    input_data = {"messages": prompt_messages, "prompt": prompt_messages}
            ...
            # If to_agent is still None, check for other fields that might indicate the target
            if to_agent is None:
                # Sometimes handoff data might be in other fields
                handoff_data = getattr(span_data, "data", {})
                if isinstance(handoff_data, dict):
                    to_agent = handoff_data.get("to_agent") or handoff_data.get("target_agent")
...
def _parse_response_span_data(span_data: Any) -> ParsedSpanData:
    ...
    try:
        ...
        if instructions or tools_info:
            ...
            elif input_data and "input" in input_data and isinstance(input_data["input"], list):
        ...
        # Manual extraction with multiple field name conventions
            ...
            if not usage_dict:
                for attr in dir(usage_obj):
                    if not attr.startswith('_') and not callable(getattr(usage_obj, attr)):
...
class OpenlayerTracerProcessor(tracing.TracingProcessor):  # type: ignore[no-redef]
    ...
    Requirements: Make sure to install the OpenAI Agents SDK with ``pip install openai-agents``.
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._current_user_inputs: Dict[str, List[str]] = {}  # Track user inputs by trace_id
        ...
        self._step_children: Dict[str, List[str]] = {}  # span_id -> list of child span_ids
        self._children_already_added: Dict[str, set] = {}  # parent_span_id -> set of added child_span_ids
        ...
        try:
            ...
            if steps_list:
                ...
                if trace.trace_id in self._current_user_inputs:
                    ...
                    if user_inputs:
                        inputs["user_query"] = user_inputs[-1]  # Use the last user input
            ...
            for span_id, step in list(self._span_to_step.items()):
                ...
                if hasattr(step, 'metadata') and step.metadata.get('trace_id') == trace.trace_id:
    ...
    def on_span_end(self, span: tracing.Span) -> None:
        ...
        try:
            ...
            if span_data:
                ...
                if trace_id and span_data:
                    ...
                    if parsed_data.span_type in ["response", "generation", "custom"]:
                        ...
                        if (parsed_data.input_data and 
                            trace_id not in self._trace_first_meaningful_input):
                            self._trace_first_meaningful_input[trace_id] = parsed_data.input_data
                        
                        # Track last meaningful output
                        if parsed_data.output_data:
                            self._trace_last_meaningful_output[trace_id] = parsed_data.output_data
            
            # Handle any orphaned children (children that were created before their parent)
            ...
            if ended_at:
                try:
                    step.end_time = datetime.fromisoformat(ended_at.replace("Z", "+00:00")).timestamp()
    ...
    def _create_step_for_span(self, span: tracing.Span, span_data: Any) -> Optional[steps.Step]:
    ...
    def _create_generation_step(self, parsed_data: ParsedSpanData, start_time: float, metadata: Dict[str, Any]) -> steps.Step:
    ...
    def _create_function_step(self, parsed_data: ParsedSpanData, start_time: float, metadata: Dict[str, Any]) -> steps.Step:
    ...
    def _create_agent_step(self, parsed_data: ParsedSpanData, start_time: float, metadata: Dict[str, Any]) -> steps.Step:
    ...
    def _create_handoff_step(self, parsed_data: ParsedSpanData, start_time: float, metadata: Dict[str, Any]) -> steps.Step:
        """Create a handoff step from HandoffSpanData."""
        from_agent = parsed_data.input_data.get("from_agent", "unknown") if parsed_data.input_data else "unknown"
    ...
    def _create_response_step(self, parsed_data: ParsedSpanData, start_time: float, metadata: Dict[str, Any]) -> steps.Step:
        ...
        if parsed_data.input_data:
            ...
            if "messages" in parsed_data.input_data:
                ...
                if user_messages:
                    inputs["user_query"] = user_messages[-1]  # Use the last user message
            ...
            if "input" in parsed_data.input_data:
                ...
                if isinstance(input_data, list) and input_data:
                    ...
                    user_content = next((msg.get("content", "") for msg in input_data if msg.get("role") == "user"), "")
                    if user_content:
        ...
        # If we still don't have good input, try to get user input from application-level capture
        ...
        output = self._extract_output_from_parsed_data(parsed_data, "Response processed")

        # Always create ChatCompletionStep for response spans - tokens will be updated in span end handler
        step = steps.ChatCompletionStep(
            name="Agent Response",
            inputs=inputs,
            output=output,
    ...
    def _extract_function_calls_from_messages(self, messages: List[Dict[str, Any]], metadata: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Check if this appears to be a cumulative conversation history vs. incremental function calls
            # Cumulative histories contain multiple different function calls from the entire conversation
            ...
            # If we have multiple different function types, this is likely cumulative conversation history
            ...
            # Create Tool Call steps for function calls that don't have corresponding function spans
    ...
    def _should_create_tool_call_step(self, function_name: str, trace_id: str) -> bool:
        ...
        We create Tool Call steps for regular tools that don't already have dedicated spans.
        We do NOT create Tool Call steps for handoff functions since they already get Handoff spans.
        """
        ...
        is_handoff_function = any(function_name.startswith(pattern) for pattern in handoff_patterns)
        
        # Do NOT create Tool Call steps for handoff functions since they already get Handoff spans
        ...
        # For now, we'll be conservative and not create Tool Call steps from message extraction
        # since regular tools already get proper function spans
        return False

    def _create_tool_call_step_from_message(self, func_data: Dict[str, Any], metadata: Dict[str, Any]) -> None:
    ...
    def _create_generic_step(self, parsed_data: ParsedSpanData, start_time: float, metadata: Dict[str, Any]) -> steps.Step:
    ...
    def _update_step_with_span_data(self, step: steps.Step, span: tracing.Span, span_data: Any) -> None:
        """Update step with final span data."""
        try:
            # Parse the span data to get the latest information including usage/tokens
            parsed_data = parse_span_data(span_data)
            
            # Extract function calls from response spans when conversation data becomes available
            if (parsed_data.span_type == "response" and 
                parsed_data.input_data and 
                "input" in parsed_data.input_data):
                input_data = parsed_data.input_data["input"]
                if isinstance(input_data, list) and input_data:
                    # Create metadata dictionary for function call extraction using helper
                    span_attrs = _extract_span_attributes(span)
                    function_metadata = {
                        **span_attrs,
                        "span_type": parsed_data.span_type,
                    }
                    self._extract_function_calls_from_messages(input_data, function_metadata)
            ...
            if parsed_data.input_data and isinstance(step, steps.ChatCompletionStep):
                ...
                should_update = (not current_inputs or 
                                current_inputs.get('response_id') == 'unknown')
                ...
                if not should_update and new_input_data:
                    ...
                    current_rich_count = sum(1 for field in rich_fields if field in current_inputs)
                    new_rich_count = sum(1 for field in rich_fields if field in new_input_data)
                    ...
                    elif 'instructions' in new_input_data and 'instructions' not in current_inputs:
                        should_update = True
            ...
            # Update function steps with input arguments when they become available
            elif parsed_data.input_data and hasattr(step, 'inputs') and parsed_data.span_type == "function":
                current_inputs = getattr(step, 'inputs', {})
                if not current_inputs or current_inputs == {}:
                    ...
                    if isinstance(step.inputs, dict) and 'input' in step.inputs and isinstance(step.inputs['input'], str):
                        try:
                            # Try to parse the JSON string
            ...
            if parsed_data.output_data:
                ...
                if updated_output and updated_output.strip():  # Check if we have meaningful content
                    # For agent spans, don't override meaningful output with generic output_data
                    if (parsed_data.span_type == "agent" and 
                        step.output and 
                        "initialized" in step.output and 
                        updated_output == "{'output_type': 'str'}"):
                    ...
                    # For response spans, always update if we have better content
                          ...
                          (step.output == "Response processed" or len(updated_output) > len(step.output))):
                        step.output = updated_output
                    # For other span types, update if it's different and not generic
                    elif updated_output != step.output and updated_output != "Response processed":
                        step.output = updated_output
                elif parsed_data.span_type == "response" and step.output == "Response processed":
            ...
            if parsed_data.span_type == "handoff" and hasattr(step, 'inputs'):
                ...
                if parsed_data.input_data:
                    ...
                    if to_agent and to_agent != 'Unknown Target' and to_agent != current_inputs.get('to_agent'):
            ...
            if isinstance(step, steps.ChatCompletionStep) and parsed_data.usage:
                ...
                if token_counts["prompt_tokens"] > 0 or token_counts["completion_tokens"] > 0:
                    step.prompt_tokens = token_counts["prompt_tokens"]
                    step.completion_tokens = token_counts["completion_tokens"]
                    step.tokens = token_counts["total_tokens"]
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            self._cleanup_dict_with_warning(self._trace_root_steps, "collected trace steps")
            self._cleanup_dict_with_warning(self._current_user_inputs, "captured user inputs")
            self._cleanup_dict_with_warning(self._trace_first_meaningful_input, "meaningful inputs")
            self._cleanup_dict_with_warning(self._trace_last_meaningful_output, "meaningful outputs")
    ...
    def _extract_output_from_parsed_data(self, parsed_data: ParsedSpanData, fallback: str = "Completed") -> str:
        """Extract output from parsed span data with consistent logic."""
        if parsed_data.output_data:
            if isinstance(parsed_data.output_data, dict) and "output" in parsed_data.output_data:
    ...
    def _extract_actual_llm_output(self, span_data: Any) -> Optional[str]:
        ...
        try:
            ...
            if hasattr(span_data, "response") and span_data.response:
                ...
                # Then check response.output for messages/function calls using helper function
                if hasattr(response, "output") and response.output:
            ...
            if isinstance(span_data, dict):
                for key in ["output", "text", "content", "message", "response", "result"]:
