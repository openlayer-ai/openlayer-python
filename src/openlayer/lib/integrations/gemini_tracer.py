    ...
    def traced_generate_content(*args, **kwargs):
        ...
        if stream:
            return handle_streaming_generate(
                *args,
                **kwargs,
                generate_func=original_generate_content,
    ...
    async def traced_generate_content_async(*args, **kwargs):
        ...
        if stream:
            return handle_streaming_generate_async(
                *args,
                **kwargs,
                generate_func=original_generate_content_async,
                inference_id=inference_id,
                model_name=client.model_name,
            )
...
def get_model_parameters(kwargs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "temperature": generation_config.get("temperature") if isinstance(generation_config, dict) else kwargs.get("temperature"),
        ...
        "max_output_tokens": generation_config.get("max_output_tokens") if isinstance(generation_config, dict) else kwargs.get("max_output_tokens"),
        "candidate_count": generation_config.get("candidate_count") if isinstance(generation_config, dict) else kwargs.get("candidate_count"),
        "stop_sequences": generation_config.get("stop_sequences") if isinstance(generation_config, dict) else kwargs.get("stop_sequences"),
    }
