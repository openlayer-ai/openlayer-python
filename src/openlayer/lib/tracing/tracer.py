    ...
    if guardrails and inputs is not None:
        try:
            ...
            modified_inputs_dict, input_guardrail_metadata = _apply_input_guardrails(
                guardrails, inputs_dict, (), {}, None
            )
            guardrail_metadata.update(input_guardrail_metadata)
            
            # Check if function should be skipped
            if hasattr(modified_inputs_dict, '__class__') and modified_inputs_dict.__class__.__name__ == 'SkipFunctionExecution':
    ...
    try:
        ...
        if guardrails and new_step.output is not None:
            try:
                final_output, output_guardrail_metadata = _apply_output_guardrails(
                    guardrails, new_step.output, inputs if isinstance(inputs, dict) else {"input": inputs}
                )
        ...
        if guardrail_metadata:
            ...
            step_metadata.update({
                ...
                "has_guardrails": True,
                "guardrail_actions": [metadata.get("action") for metadata in guardrail_metadata.values()],
                "guardrail_names": [
...
def add_chat_completion_step_to_trace(guardrails: Optional[List[Any]] = None, **kwargs) -> None:
    """Adds a chat completion step to the trace with optional guardrails.
    
    Args:
        guardrails: Optional list of guardrail instances to apply. If None, uses configured global guardrails.
            ...
            with create_step(*step_args, inference_pipeline_id=inference_pipeline_id, **step_kwargs) as step:
                ...
                try:
                    ...
                    modified_inputs, input_guardrail_metadata = _apply_input_guardrails(
                        guardrails or [], original_inputs, func_args, func_kwargs, func_signature
                    )
                    guardrail_metadata.update(input_guardrail_metadata)
                    
                    # Check if function execution should be skipped
                    if hasattr(modified_inputs, '__class__') and modified_inputs.__class__.__name__ == 'SkipFunctionExecution':
                        # Function execution was blocked with SKIP_FUNCTION strategy
                        output = None
                        logger.debug("Function %s execution skipped by guardrail", func.__name__)
                    else:
                        ...
                        if modified_inputs != original_inputs:
                            ...
                            for param_name, modified_value in modified_inputs.items():
                    ...
                    if hasattr(modified_inputs, '__class__') and modified_inputs.__class__.__name__ == 'SkipFunctionExecution':
                        ...
                        # Use original inputs for logging since modified_inputs is a special marker
                        modified_inputs = original_inputs
                    else:
                        final_output, output_guardrail_metadata = _apply_output_guardrails(
                            guardrails or [], output, modified_inputs or original_inputs
                        )
                    guardrail_metadata.update(output_guardrail_metadata)
                    ...
                    with create_step(
                        ...
                        **step_kwargs,
                        ...
                        _process_wrapper_inputs_and_outputs(
                            ...
                            context_kwarg=context_kwarg,
                            output=output,
                            guardrail_metadata={},  # TODO: Add guardrail support for async functions
                        )
...
def _handle_guardrail_block(
    ...
    if strategy == BlockStrategy.RAISE_EXCEPTION:
        ...
        logger.info("Guardrail %s blocked %s, returning error message", guardrail.name, 'input' if is_input else 'output')
        ...
        logger.info("Guardrail %s blocked %s, skipping execution", guardrail.name, 'input' if is_input else 'output')
