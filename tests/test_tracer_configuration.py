...
class TestTracerConfiguration:
    ...
    def test_get_client_with_ssl_disabled_and_config(self, mock_openlayer: Any, mock_http_client: Any) -> None:
        ...
        with patch.object(tracer, '_publish', True), \
             patch.object(tracer, '_verify_ssl', False):
    ...
    def test_handle_trace_completion_uses_configured_pipeline_id(self, mock_utils: Any) -> None:
        ...
        with patch.object(tracer, '_publish', True), \
             patch.object(tracer, '_get_client') as mock_get_client:
            ...
            with patch.object(tracer, 'get_current_trace') as mock_get_trace, \
                 patch.object(tracer, 'post_process_trace') as mock_post_process:
        ...
        with patch.object(tracer, '_publish', True), \
            ...
            with patch.object(tracer, 'get_current_trace') as mock_get_trace, \
                ...
                tracer._handle_trace_completion(
                    is_root_step=True,
                    step_name="test_step",
                    inference_pipeline_id="provided_pipeline_id"
                )
